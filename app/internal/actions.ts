...
export async function fetchSystemAgentsAction() {
  try {
    const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000";
    const res = await fetch(`${baseUrl}/api/system/agents`, { cache: "no-store" });
    if (!res.ok) {
    ...
    // Static fallback documentation if fetch fails during server actions execution
    ...
    return { success: true, agents: fallbackAgents };
  }
}

export async function testCommunityScoreAction(candidateId: string) {
  try {
    if (!candidateId) {
      throw new Error("Candidate ID is required.");
    }
    const result = await getOrComputeCommunityReputation(candidateId, true);
...
export async function fetchTalentHeatmapAction(groupBy: string = "campus") {
  try {
    const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000";
    const res = await fetch(`${baseUrl}/api/analytics/heatmap?by=${groupBy}`, { cache: "no-store" });
    if (!res.ok) {
      throw new Error(`HTTP ${res.status}`);
    }
    const data = await res.json();
    return { success: true, heatmap: data };
  } catch (err: any) {
    return { success: false, error: err.message };
  }
}

export async function fetchRecruiterAnalyticsAction() {
  try {
    const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000";
    const res = await fetch(`${baseUrl}/api/recruiter/analytics`, { cache: "no-store" });
    if (!res.ok) {
      throw new Error(`HTTP ${res.status}`);
    }
    const data = await res.json();
